...
async def load_function_from_url(request: Request, form_data: LoadUrlForm, user=Depends(get_admin_user)):
    ...
    if _parsed.scheme not in ('http', 'https') or _parsed.hostname not in ALLOWED_FUNCTION_URL_HOSTS:
        raise HTTPException(status_code=400, detail='URL host is not allowed. Only GitHub and GitLab sources are permitted.')
