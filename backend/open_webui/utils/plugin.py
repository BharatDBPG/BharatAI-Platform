...
async def load_tool_module_by_id(tool_id, content=None):
    ...
    try:
        ...
        if not ENABLE_RISK_TOOL_EXEC:
            raise Exception('Tool/plugin execution is disabled in this deployment (ENABLE_RISK_TOOL_EXEC=False). Contact administrator.')
...
async def load_function_module_by_id(function_id: str, content: str | None = None):
    ...
    try:
        ...
        if not ENABLE_RISK_TOOL_EXEC:
            raise Exception('Function/plugin execution is disabled in this deployment (ENABLE_RISK_TOOL_EXEC=False). Contact administrator.')
