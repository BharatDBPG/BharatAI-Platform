...
PARICHAY_CLIENT_ID = ConfigVar(
    'PARICHAY_CLIENT_ID',
    'oauth.parichay.client_id',
    os.getenv('PARICHAY_CLIENT_ID', ''),
)

PARICHAY_CLIENT_SECRET = ConfigVar(
    'PARICHAY_CLIENT_SECRET',
    'oauth.parichay.client_secret',
    os.getenv('PARICHAY_CLIENT_SECRET', ''),
)

PARICHAY_API_URL = ConfigVar(
    'PARICHAY_API_URL',
    'oauth.parichay.api_url',
    os.getenv('PARICHAY_API_URL', 'https://parichay.staging.nic.in'),
)

PARICHAY_REDIRECT_URI = ConfigVar(
    'PARICHAY_REDIRECT_URI',
    'oauth.parichay.redirect_uri',
    os.getenv('PARICHAY_REDIRECT_URI', ''),
)
...
def load_oauth_providers():
    ...
    if PARICHAY_CLIENT_ID.value and PARICHAY_CLIENT_SECRET.value:

        def parichay_oauth_register(oauth: OAuth):
            client = oauth.register(
                name='parichay',
                client_id=PARICHAY_CLIENT_ID.value,
                client_secret=PARICHAY_CLIENT_SECRET.value,
                access_token_url=f"{PARICHAY_API_URL.value.rstrip('/')}/pnv1/salt/api/oauth2/token",
                authorize_url=f"{PARICHAY_API_URL.value.rstrip('/')}/pnv1/oauth2/authorize",
                api_base_url=f"{PARICHAY_API_URL.value.rstrip('/')}",
                client_kwargs={
                    'scope': 'user_details',
                    'code_challenge_method': 'S256',
                    **({'timeout': int(OAUTH_TIMEOUT.value)} if OAUTH_TIMEOUT.value else {}),
                },
                redirect_uri=PARICHAY_REDIRECT_URI.value,
            )
            return client

        OAUTH_PROVIDERS['parichay'] = {
            'name': 'Parichay',
            'register': parichay_oauth_register,
            'sub_claim': 'parichayId',
        }
    ...
    if PARICHAY_CLIENT_ID.value:
        configured_providers.append('Parichay')

    # Parichay handles its own logout — skip the OPENID_PROVIDER_URL warning for it
    warn_providers = [p for p in configured_providers if p != 'Parichay']
    if warn_providers and not OPENID_PROVIDER_URL.value and not OPENID_END_SESSION_ENDPOINT.value:
        provider_list = ', '.join(warn_providers)
        log.warning(
